...
int main(int argc, char* argv[])
{
	...
	*((int*)(CPUString + 8)) = CPUInfo[2];
			...
			bLBRVisualization || bFP128 || bMOVOptimization)
		{
			...
			while (i < (sizeof(szFeatures) / sizeof(const char*)))
			{
				if (nFeatureInfo & nIds)
				{
					...
					if (szFeatures[i] == "MMX Technology")
						isMMX = true;

					if (szFeatures[i] == "SSE Extensions")
						isSSE = true;

					if (szFeatures[i] == "SSE2 Extensions")
						isSSE2 = true;
	...
	if (!(isMMX && isSSE && isSSE2)) {
		std::cout << "MMX, SSE, or SSE2 not supported" << std::endl;
		return 0;
	}
